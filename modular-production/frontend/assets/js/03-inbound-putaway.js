...
function applyBinChange(palletIdx){
  ...
  const oldBm = binMaster.find(b=>b.id===row.binId);
...
function saveGRN(){
  ...
  callSheet('saveGRNToSheet', entry,
    r=>{ if(r&&r.success) showResult('grnResult','GRN #'+no+' saved and synced to Google Sheet.','success'); },
    ()=>{}
  );
}
  ...
  el.innerHTML=putawayPlan.map((r,i)=>{
    ...
    return groupHeader+`<div class="putaway-row" style="${r.warning?'border-color:#fecaca;background:#fff8f8':''}">
      <div class="putaway-bin" style="display:flex;flex-direction:column;align-items:center;gap:6px">
        ...
        <button onclick="changePalletBin(${i})" style="font-size:9px;padding:2px 7px;border:1px solid var(--border);border-radius:4px;background:var(--surface);cursor:pointer;color:var(--slate3)" title="Change bin for this pallet">✎ Change</button>
      </div>
