...
async function callSheet(action, payload, onSuccess, onError) {
  ...
  if (typeof google !== 'undefined' && google.script) {
    ...
    else if (action === 'savePutawayPlan')      runner.savePutawayPlan(payload.rows, payload.grnNo, payload.createdBy);
    else if (action === 'loadActivePutawayPlan') runner.loadActivePutawayPlan();
    else if (action === 'confirmPutawayRow')    runner.confirmPutawayRow(payload.planId, payload.lineNo, payload.binId, payload.confirmedBy);
    else if (action === 'swapPutawayBin')       runner.swapPutawayBin(payload.planId, payload.lineNo, payload.oldBinId, payload.newBinId, payload.updatedBy);
    else if (action === 'cancelPutawayPlan')    runner.cancelPutawayPlan(payload.planId, payload.releasedBy);
    else if (action === 'confirmAllPutaway')    runner.confirmAllPutaway(payload.planId, payload.confirmedBy);
    else if (action === 'updateBinStatus')      runner.updateBinStatus(payload.binId, payload.status, payload.updatedBy);
    else if (action === 'addOrUpdateSKURow')    runner.addOrUpdateSKURow(payload);
    else if (action === 'addOrUpdateBinRow')    runner.addOrUpdateBinRow(payload);
