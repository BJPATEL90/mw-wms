...
function initExpiryWatch() {
  ...
  function parseExp(s) {
    ...
    if (m) return new Date(+m[3], +m[2]-1, +m[1]);
    // dd/mm/yyyy  ← Uniware common format
  ...
  // Aggregate by bin+sku+batch — ALL stock types included so DAMAGE/expired stock is visible
  var agg = {};
  (inventory || []).forEach(function(r) {
    ...
    if (days > 90) return; // show expired (days<0) and within-90-day near-expiry
    var key = r.bin + '|||' + r.skuCode + '|||' + (r.batch||'');
    if (!agg[key]) agg[key] = {
      skuCode:r.skuCode, skuName:r.skuName||'', batch:r.batch||'',
      expDate:r.expDate, bin:r.bin, days:days,
      brand:brandOf(r.skuCode), stockType:r.stockType||'GOOD', qty:0
    };
    agg[key].qty += (r.qty||0);
  });
...
function renderExpiryWatch() {
  ...
  body.innerHTML = rows.map(function(r, i) {
    ...
    return '<tr style="background:' + (isExp ? '#fff8f8' : '#fffbeb') + '">'
      ...
      + '<td style="text-align:right;font-weight:700">' + r.qty.toLocaleString() + '</td>'
      + '<td><span style="font-size:10px;font-weight:600;padding:2px 6px;border-radius:4px;background:' + (r.stockType==='GOOD'||r.stockType==='UNRESTRICTED'?'#dcfce7':'#fee2e2') + ';color:' + (r.stockType==='GOOD'||r.stockType==='UNRESTRICTED'?'#166534':'#991b1b') + '">' + (r.stockType||'GOOD') + '</span></td>'
      + '</tr>';
  }).join('');
}
